<template>
  <v-container grid-list-md>
    <v-data-iterator
      ...
      item-key="id"
    >
      <template v-slot:no-data>asdas</template>
                ...
                <v-card class="fill-height">
                  ...
                  <v-overlay v-if="group.archived_at" absolute color="#3c3c3c">
                    <v-layout row>
                      ...
                      <v-btn icon @click="confirmAndRemove(group.id)">
...
export default {
  ...
  methods: {
    ...
    async confirmAndRemove(group_id) {
      ...
      if (confirmed) {
        try {
          await this.remove(group_id);
          this.$toast.success(this.$t("group.remove.success"));
        } catch (error) {
